...
#include "mcp794xx_driver/src/mcp794xx_driver_basic.h"
...
#define EXTINT_IRQ_PIN				6

int main()
{
    stdio_init_all();

    /**< iniotialize builtin led port*/
    gpio_init(PICO_DEFAULT_LED_PIN);
    gpio_set_dir(PICO_DEFAULT_LED_PIN, GPIO_OUT);

	/**< initialize the external irq gpio pin */
	gpio_init(EXTINT_IRQ_PIN);
	gpio_set_dir(EXTINT_IRQ_PIN, GPIO_OUT);
	gpio_pull_up(EXTINT_IRQ_PIN);

	gpio_set_irq_enabled_with_callback(EXTINT_IRQ_PIN, 0xb0010, 1, mcp794xx_interface_irq_callback);       /**< enable irq on pin falling edge*/
    ...
    while (1)
    {
        switch((int)test.state)
		{
			...
			case TEST_ALARM:
			{
				...
				{
					gpio_put(PICO_DEFAULT_LED_PIN, !gpio_get(PICO_DEFAULT_LED_PIN));										/**< toggle user led for debug purposes */;											
			...
			{   /**< !! note that week day must be set accurately according to calendar for this routine to work !! */
				...
				if(alarm_flag == 1)    														/**< this routine should be executed inside the external interrupt callback function */
				{	
					// led_toggle();															/**< toggle user led for debug purposes */
			...
			case TEST_POWER_FAIL:
			{
				...
				{
					pwr_fail_read = true;
					mcp794xx_basic_get_pwr_fail_time_stamp(MCP794XX_PWR_DOWN_TIME_STAMP, &pwr_fail);         /**< read power fail time stamp */
...
uint8_t i2c_write(uint8_t addr, uint8_t *pBuf, uint8_t len)
{
 i2c_write_blocking(i2c_default, addr, (uint8_t*)pBuf, len, false);
}
